...
V8ClientConnection::V8ClientConnection(
    ...
    : _databaseName(database),
      ...
      _version("arango"),
      _mode("unknown mode") {
  _client.reset(new SimpleHttpClient(connection, requestTimeout, false));
...
static void ClientConnection_reconnect(
  ...
  std::string const oldEndpoint = client->endpoint();
  std::string const oldDatabaseName = client->databaseName();
  std::string const oldUsername = client->username();
  std::string const oldPassword = client->password();
  ...
  try {
    connection = client->createConnection(endpoint);
  } catch (...) {
    client->setEndpoint(oldEndpoint);
    client->setDatabaseName(databaseName);
  ...
  V8ClientConnection* newConnection =
      CreateV8ClientConnection(connection, client);

  if (newConnection->isConnected() &&
      newConnection->lastHttpReturnCode() == HttpResponse::OK) {
    LOG(INFO) << "Connected to ArangoDB "
              << "'" << v8connection->endpointSpecification() << "', "
              << "version " << v8connection->version() << " ["
              << v8connection->mode() << "], "
    ...
    LOG(ERR) << "Could not connect to endpoint '" << endpoint
             << "', username: '" << client->username() << "'";

    std::string errorMsg = "could not connect";

    if (!newConnection->lastErrorMessage().empty()) {
      errorMsg = newConnection->lastErrorMessage();
    }
